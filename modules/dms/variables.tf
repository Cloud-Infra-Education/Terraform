...
variable "create_iam_roles" {
  description = "DMS 필수 IAM Role(dms-vpc-role, dms-cloudwatch-logs-role) 를 생성. 이미 계정에 존재하면 false 권장"
  type        = bool
  default     = true
}

variable "vpc_id" {
  description = "DMS replication instance가 배치될 VPC ID"
  type        = string
}

variable "subnet_ids" {
  description = "DMS replication subnet group에 포함할 subnet IDs (보통 private subnets)"
  type        = list(string)
}

variable "target_db_security_group_id" {
  description = "Target DB 보안그룹 ID. 지정 시 DMS SG -> DB 포트 인그레스 룰을 자동 추가"
  type        = string
  default     = null
}
